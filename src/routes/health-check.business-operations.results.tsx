...
export const Route = createFileRoute("/health-check/business-operations/results")({
  head: () => ({
    meta: [
      { title: "Your Business Operations Health Check Results | Simple Secure Solutions" },
...
function Results() {
  const stored = typeof window !== "undefined" ? localStorage.getItem("sss-business-operations-last") : null;
  const data = useMemo(() => {
    try {
      return stored
        ? (JSON.parse(stored) as { businessName?: string; contactName?: string; email?: string; phone?: string; answers: AnswerMap })
        : null;
    } catch {
      return null;
    }
  ...
  const areas = useMemo(() => getAreas(data?.answers ?? {}), [data]);
      ...
      <main className="pt-28 pb-24">
        <section className="relative overflow-hidden bg-navy text-navy-foreground">
          ...
          <div className="relative mx-auto max-w-6xl px-6 py-14 lg:px-10 lg:py-16">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.16em] text-white/70">
              <ClipboardCheck className="h-3.5 w-3.5 text-copper" />
              Health Check System
            </div>
            ...
            <div className="mt-10 grid gap-6 rounded-3xl border border-white/12 bg-white/[0.04] p-6 backdrop-blur-sm sm:p-8 lg:grid-cols-[auto_1fr] lg:items-center lg:gap-10">
              ...
              <div>
                ...
                <p className="mt-2 font-display text-2xl text-white sm:text-3xl">{result.label}</p>
                ...
                <p className="mt-5 inline-flex items-start gap-2 rounded-xl border border-white/12 bg-white/[0.04] px-3.5 py-2.5 text-xs leading-relaxed text-white/60">
                  This Health Check highlights potential areas of interest based on your answers. It is a high-level view, not a full operational audit.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="border-y border-border bg-surface">
          <div className="mx-auto max-w-6xl px-6 py-16 lg:px-10">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-copper">What’s working well</p>
            <h2 className="mt-3 font-display text-2xl text-navy sm:text-3xl">Strengths in your current operations</h2>
            <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-muted-foreground">These are the operational foundations your answers suggest are working relatively well today.</p>
            <div className="mt-8 grid gap-5 lg:grid-cols-2">
              {areas.filter((area) => area.status === "good").map((area) => (
                <article key={area.id} className="relative overflow-hidden rounded-2xl border border-border bg-card p-6 shadow-soft">
                  <span className="absolute inset-y-0 left-0 w-1 bg-success" aria-hidden />
                  <div className="flex gap-3">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-success" />
                    <div><p className="text-sm font-semibold text-navy">{area.label}</p><p className="mt-2 text-sm leading-relaxed text-foreground/80">{area.detail}</p></div>
                  </div>
                </article>
              ))}
              {areas.every((area) => area.status !== "good") && (
                <article className="relative overflow-hidden rounded-2xl border border-border bg-card p-6 shadow-soft lg:col-span-2">
                  <p className="text-sm leading-relaxed text-foreground/80">Your answers indicate there are opportunities across several operational areas. Improving the foundations first can make future process changes easier to sustain.</p>
                </article>
              )}
            </div>
          </div>
        ...
        <section className="border-y border-border bg-surface">
          <div className="mx-auto max-w-6xl px-6 py-16 lg:px-10">
            ...
            <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-muted-foreground">These areas may benefit from additional attention based on your answers.</p>
            <div className="mt-8 grid gap-5 lg:grid-cols-2">
              {result.opportunities.map((item) => (
                <article key={item} className="relative overflow-hidden rounded-2xl border border-border bg-card p-6 shadow-soft">
                  ...
                  <div className="flex gap-3"><CircleAlert className="mt-0.5 h-4 w-4 shrink-0 text-copper" /><p className="text-sm leading-relaxed text-foreground/80">{item}</p></div>
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="mx-auto max-w-6xl px-6 py-16 lg:px-10">
          ...
          <h2 className="mt-3 font-display text-2xl text-navy sm:text-3xl">Business Operations findings</h2>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {areas.map((area) => (
              <div key={area.id} className="rounded-2xl border border-border bg-card p-6 shadow-soft">
                <div className="flex items-start justify-between gap-3">
                  <div><p className="text-base font-semibold text-navy">{area.label}</p><p className="mt-1 text-xs text-muted-foreground">{statusLabel(area.status)}</p></div>
                ...
                <div className="mt-5 h-2 overflow-hidden rounded-full bg-secondary"><div className="h-full rounded-full bg-copper" style={{ width: `${Math.min(100, area.score * 10)}%` }} /></div>
                <div className="mt-4 flex gap-2 text-xs leading-relaxed text-muted-foreground"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-copper" />{area.detail}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-6 pb-16 lg:px-10">
          <div className="relative overflow-hidden rounded-3xl bg-navy p-8 text-navy-foreground sm:p-12">
            ...
            <div className="relative max-w-2xl">
              ...
              <p className="mt-4 text-[15px] leading-relaxed text-white/75">Simple Secure Solutions can review the areas identified in this assessment and help you prioritize practical improvements such as workflow systems, SOPs and documentation, process automation, customer enquiry tracking, onboarding and operational support.</p>
              <div className="mt-6 flex flex-wrap gap-3">
                <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-full bg-copper px-5 py-3 text-sm font-medium text-copper-foreground shadow-copper transition-colors hover:bg-copper/90">Book a Consultation <Sparkles className="h-4 w-4" /></a>
                <Link to="/health-check/business-operations/" className="inline-flex items-center gap-1.5 rounded-full border border-white/20 px-5 py-3 text-sm font-medium text-white/85 transition-colors hover:bg-white/10"><ArrowLeft className="h-4 w-4" />Retake the Health Check</Link>
              </div>
            </div>
          </div>
        </section>

        <div className="mx-auto flex max-w-6xl flex-wrap gap-4 px-6 lg:px-10"><Link to="/" className="text-sm font-medium text-muted-foreground hover:text-navy">Back to Simple Secure Solutions</Link></div>
      </main>
    </div>
  );
}
...
function area(id: string, label: string, questionIds: string[], answers: AnswerMap, detail: string) {
  ...
  return { id, label, status, score: average, detail };
}
...
function ScoreDial({ score }: { score: number }) {
  ...
  return (
    <div className="relative h-48 w-48 shrink-0">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center"><span className="font-display text-4xl leading-none text-white">{score}</span><span className="mt-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-white/60">/ 100</span></div>
    </div>
  );
}
